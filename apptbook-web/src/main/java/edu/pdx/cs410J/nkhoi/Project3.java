...
public class Project3 {
    ...
    static final String README = "KHOI NGUYEN - PROJECT 3 - This project will take owner name, description, begintime, and ending time to create an appointment. " +
            "The program will add that appointment to the Appointment Book with the [option] to print that appointment. Additionally, with the " +
            ...
            "This project is different than Project 2 that user can pretty print the contents of the entire appointment " +
            "book to either a file or standard out depends on the option follow '-pretty' ";
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length && !flag; i++) {
            ...
            if (!args[i].equals("-print") && !args[i].equals("-README") && !args[i].equals("-textFile") && !args[i].equals(fileName) && !args[i].equals("-pretty") && !args[i].equals(prettyfileName)) {
                if (args.length - i == 8) {
                    FromCommandLine = new Appointment(args[i + 1], args[i + 2], args[i + 3], args[i + 4], args[i + 5], args[i + 6], args[i + 7]);
        ...
        if (isPrettyOpt) {
            if (prettyfileName.equals("-")) {
                ...
                if (FileExist == 2)
                    ...
                    infile.prettydisplay();
            } else {
                try {
                    PrettyPrinter file = new PrettyPrinter(prettyfileName);
                    file.dump(FileExist == 2 ? temp : infile);
